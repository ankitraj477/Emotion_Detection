...
def predict_audio_emotion(file_path):
    try:
        ...
        audio_model = tf.keras.models.load_model(loaded_model_json)

        predicted_emotion = audio_model.predict(mfccs)
        ...
        print("Error predicting audio emotion:", e)
...
def upload_image():
    try:
        ...
        uploaded.thumbnail(
            ((top.winfo_width()/1.75), (top.winfo_height()/1.75)))
...
def upload_audio():
    file_path = filedialog.askopenfilename()
    predicted_emotion = predict_audio_emotion(file_path)
    audio_emotion_label.configure(
        text="Predicted Audio Emotion: " + predicted_emotion)


audio_upload_button = Button(top, text="Upload Audio", command=upload_audio)
audio_upload_button.pack(side='bottom', pady=50)

audio_emotion_label = Label(top, text="")

audio_emotion_label.pack(side='bottom', expand='True')
